 ...
 */
  ...
  window.addEventListener("load", function() {
    ...
    for (let i = 1; i < words.length; i++) {
      ...
      //Keep the top 100 words in the unique array
      unique = unique.slice(0, 100);

      // Find the highest duplicate count (most repeated word)
      let maxCount = unique[0][1];

      // Sort the unique array alphabetically by word (unique[i][0])
      unique.sort();

      // Reference the word cloud box
      let cloudBox = document.getElementById("wc_cloud");

      // Clear any previous content in the word cloud area
      cloudBox.innerHTML = "";

      // Size each word based on its usage (frequency)
      for (let i = 0; i < unique.length; i++) {
          let word = document.createElement("span");
          word.textContent = unique[i][0];  // The word itself
          word.style.fontSize = (unique[i][1] / maxCount) + "em";  // Scale size by frequency
          cloudBox.appendChild(word);  // Add to the display area
      }
